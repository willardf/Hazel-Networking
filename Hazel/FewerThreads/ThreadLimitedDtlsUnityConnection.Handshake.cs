...
namespace Hazel.Dtls
{
    ...
    /// </summary>
    /// <inheritdoc />
    public partial class ThreadLimitedDtlsUnityConnection
    {
        /// <summary>
        ...
        /// <inheritdoc />
        private void ResendPacketsIfNeeded()
        {
            lock (this.syncRoot)
            {
        ...
        /// </returns>
        private ByteSpan CreateApplicationDataRecord(byte[] bytes, int length)
        {
            lock (this.syncRoot)
            {
        ...
        /// <inheritdoc />
        protected void EncryptAndSendPlaintext(byte[] bytes, int length)
        {
            ByteSpan wireData = this.CreateApplicationDataRecord(bytes, length);
            if (wireData.Length > 0)
            {
                Debug.Assert(wireData.Offset == 0, "Got a non-zero write data offset");
                this.sendQueue.TryAdd(wireData);
            }
        }
        ...
        protected internal override void HandleReceive(MessageReader reader, int bytesReceived)
        {
            ...
            lock (this.syncRoot)
            {
                this.DecryptIncomingRecords(message);
            }
        ...
        private void DecryptIncomingRecords(ByteSpan span)
        {
            ...
            while (span.Length > 0)
            {
                ...
                if (record.SequenceNumber < this.currentEpoch.NextExpectedSequence)
                {
                    if (windowIndex >= 64)
                    {
                        this.logger.WriteError($"Dropping too-old record: Sequence({record.SequenceNumber}) Expected({this.currentEpoch.NextExpectedSequence})");
                        continue;
                    }
